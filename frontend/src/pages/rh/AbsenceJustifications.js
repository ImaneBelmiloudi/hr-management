...
const AbsenceJustifications = () => {
  ...
  const handleReject = async (id) => {
    try {
      ...
      await api.post(`/api/absence-justifications/${id}/status`, {
        status: 'rejected',
        rejection_reason: 'Justification rejected by HR' // Default rejection reason
      });
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {filteredJustifications.map((justification) => (
                  <tr key={justification.id}>
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{formatDate(justification.start_date)}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{formatDate(justification.end_date)}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
